...
def read_audio(ws, timeout):
    ...
    authenticator1 = IAMAuthenticator('DnpiU5pEnARjiAIF9DD2M8Jrl6_EOCn5MIQX3nEOf1sw')
    ...
    response = assistant.message_stateless(assistant_id='8f2ddd04-4b59-4fb0-90be-5563f6045134', input={
        'message_type': 'text',
        'text': txt
    }).get_result()

    output_json = json.dumps(response, indent=2)
    output_dict = json.loads(output_json)
    response_type = jmespath.search("output.generic[0].response_type", output_dict)
    global assistant_replay
    with open("assistant_replay.txt", "w") as wr:
        if response_type == "option":
            dict = jmespath.search("output.generic[0].options[*].label", output_dict)
            print("\nWatson Assistant: ")
            for option in dict:
                wr.write(option)
                print(option, '\n')
            ...
            print("\nWatson Assistant: ")
            assistant_replay = jmespath.search("output.generic[0].text", output_dict)
            wr.write(assistant_replay)
            print(assistant_replay)
    ...
    with open('assistant_replay.txt', 'r') as f:
        text = f.readlines()
    text = [line.replace('\n', '') for line in text]
